...
Hence, we extract only the largest component by area.
...
\begin{figure}[H]
    \centering
    \fbox{\includegraphics[scale=0.9]{{DEBUG_bin_tr06}.jpg}}
...
allowed, as well as the ratio is what we expect the digits to be around.

The idea for the second and third stages are ideas that I've come up with and
implemented fully.


\begin{figure}[H]
    \centering
    \fbox{\includegraphics[scale=0.9]{{DEBUG_contours_tr06}.jpg}}
    \caption{The contour groups extracted from the image}
\end{figure}

\begin{figure}[H]
    \centering{
        \fbox{\includegraphics[scale=0.9]{{DEBUG_cropped_0_tr06}.jpg}}
        \fbox{\includegraphics[scale=0.9]{{DEBUG_cropped_1_tr06}.jpg}}
    }
...
This aggressive nature brings forth issues where the filtering is overfitting
the provided images given to extract the digits. Overfitting in a way where it
works best on the provided images, but works less optimally on other images of
similar types. Under extended testing of extra images of house numbers searched
on the internet, it is observed that it is able to extract some digits, fail to
extract others and included some noise along with the extracts.
