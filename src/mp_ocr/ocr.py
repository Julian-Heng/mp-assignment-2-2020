...
class OCR:
    ...
    def detect(image, knn, debug=False):
        ...
        processed_img = OCR._preprocess_image(image)
        contours = OCR._extract_contours(processed_img, image)
        cropped = OCR._crop_contour_group(image, contours)
    ...
    def _extract_contours(processed_img, image):
        ...
        stages = {
            "Stage 1": ("Filtering bad contours", OCR._stage_1_contour_filter),
            "Stage 2": ("Comparing contours distance", OCR._stage_2_contour_filter),
            "Stage 3": (
                "Comparing contours line straightness",
                OCR._stage_3_contour_filter,
            ),
        }
        ...
        for stage_name, (stage_msg, stage_action) in stages.items():
            ...
            msg = f"{stage_name}: Removed {delta} contours, {len(filtered_contours)} remains. ({elapsed:.2f}ms)"
            logging.debug(msg)
    ...
    def _stage_1_contour_filter(contours, image):
        msg_fmt = "Contour {} is good"
        filtered_msg_fmt = "Contour {} filtered: {}"
        ...
        for i, contour in enumerate(contours):
            ...
            if h < w:
                msg = filtered_msg_fmt.format(i, "height is less than width")

            # Filter any contours where the height is less than 1.36 times the
            # width
            elif contour_aspect_ratio < 1.36:
                msg = filtered_msg_fmt.format(
                    i,
                    f"contour aspect ratio is less than 1.36 ({contour_aspect_ratio:.5f})",
                )
            ...
            # width
                ...
                msg = filtered_msg_fmt.format(
                    i,
                    f"contour aspect ratio is more than 3.4 ({contour_area_ratio:.5f})",
                )
            ...
            # height
                ...
                msg = filtered_msg_fmt.format(
                    i,
                    f"contour height to image height ratio is more than 0.75 ({height_ratio:.5f})",
                )
            ...
            # width
                ...
                msg = filtered_msg_fmt.format(
                    i,
                    f"contour width to image width ratio is more than 0.75 ({width_ratio:.5f})",
                )

            # Filter any contours where the number of pixels is less than 70
            elif contour_area < 70:
                msg = filtered_msg_fmt.format(
                    i, f"contour area is less than 70 ({contour_area})"
                )
            ...
            # image
                ...
                msg = filtered_msg_fmt.format(
                    i,
                    f"contour image area ratio is more than 0.08 ({contour_area_ratio:.5f})",
                )
            ...
            # image
                ...
                msg = filtered_msg_fmt.format(
                    i,
                    f"contour image area ratio is less than 0.00134 ({contour_area_ratio:.5f})",
                )
            ...
            # than 10
                ...
                msg = filtered_msg_fmt.format(
                    i,
                    f"contour approximate datapoints count is less than 10 ({num_approx})",
                )
            ...
            # than 1000
                ...
                msg = filtered_msg_fmt.format(
                    i,
                    f"contour approximate datapoints count is more than 1000 ({num_approx})",
                )
            ...
            # Filter any contours where they lie on the edges of the image
                ...
                msg = filtered_msg_fmt.format(i, "contour is on the edge of the image")

            # Passed all checks
            else:
                msg = msg_fmt.format(i)
    ...
    def _stage_2_contour_filter(contours, image):
        ...
        for i, j in combinations(range(len(contours)), 2):
            ...
            # Get the smallest possible points between all points of the 2
        ...
        for i, j, distance in distances:
            ...
            dest_group = next(
                (g for g in groups if any(i in c or j in c for c in g)), None
            )
        ...
        # coordinates. This is an edge case, so we would need to treat each
        # contour as it's own group.
        if len(groups) == 0:
            for i, j, _ in distances:
                # We can link each index to itself because they will be trimmed
                # out
                groups.append([(i, i)])
                groups.append([(j, j)])

        # Convert group indicies to contours
        groups = OCR._map_group_indexes_to_contours(groups, contours)

        # Get the largest contour group
        largest_group = OCR._largest_contour_group_by_area(groups)
    ...
    def _stage_3_contour_filter(contours, image):
        ...
        for i, j in combinations(range(len(contours)), 2):
            ...
            # Get the angle of the gradient of the line connecting the two
            # points
        ...
        for i, j, angle in angles:
            ...
            if not utils.is_around(angle, 0, 20) and not utils.is_around(
                angle, 180, 20
            ):
